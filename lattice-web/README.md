...
├── worker/            # Redis consumer: clone (auth optional) + lattice
...
- **Scan Execution**: Queue scans to Redis; worker clones (authenticated when needed) and runs lattice
...
GitHub App env: `GITHUB_APP_ID`, `GITHUB_APP_SLUG`, `GITHUB_APP_PRIVATE_KEY` (or `_PATH`), callback `LATTICE_PUBLIC_URL/api/v1/vcs/github/callback`. Local default: `GITHUB_APP_MOCK=1`.
...
- `LATTICE_PUBLIC_URL`, `LATTICE_UI_URL` — callback and post-install redirects
...
### Worker
- `LATTICE_PATH` - Path to lattice binary
