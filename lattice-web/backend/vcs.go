...
func uiURL() string {
	if u := strings.TrimSpace(os.Getenv("LATTICE_UI_URL")); u != "" {
...
func publicAPIURL() string {
	if u := strings.TrimSpace(os.Getenv("LATTICE_PUBLIC_URL")); u != "" {
		return strings.TrimRight(u, "/")
	}
...
func (v *vcsService) githubListRepos(conn *models.VcsConnection) ([]RemoteRepo, error) {
	if mockGitHubApp() {
		return []RemoteRepo{
			{
				ID:            "1",
				FullName:      "lattice-demo/vulnerable-sample",
			...
			{
				ID:            "2",
				FullName:      "lattice-demo/bandit-examples",
