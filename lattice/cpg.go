...
const (
	...
	cpgRunnerPackage = "com.lattice.security.Runner"
)

var cpgRunnerSource = filepath.Join("cpg", "src", "main", "java", "com", "lattice", "security", "Runner.java")
...
func cpgHomeDir() (string, error) {
	...
	return filepath.Join(home, ".lattice", "cpg", cpgVersion), nil
}
...
func bundledJavaHomeDir() (string, error) {
	...
	return filepath.Join(home, ".lattice", "java", fmt.Sprintf("jdk-%d", minJavaVersion)), nil
}
...
func downloadJavaJDK() (string, string, error) {
	...
	tempDir, err := os.MkdirTemp("", "lattice-java-*")
	if err != nil {
		return "", "", err
	}
...
func hasSupportedCpgSources(codeDir string) (bool, error) {
	...
	err := filepath.WalkDir(codeDir, func(path string, d os.DirEntry, err error) error {
		...
		if d.IsDir() {
			name := d.Name()
			if name == ".git" || name == ".lattice" || name == "node_modules" {
...
func httpGetWithRetry(url string) (*http.Response, error) {
	...
	for i := 0; i < 5; i++ {
		...
		req.Header.Set("User-Agent", "lattice/1.0.0 (https://github.com/marcinguy/checkmate-go)")
