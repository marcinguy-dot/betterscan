...
func TestCpgHomeDir(t *testing.T) {
	...
	expected := filepath.Join(home, ".lattice", "cpg", cpgVersion)
	got, err := cpgHomeDir()
	if err != nil {
...
func TestCpgRunnerPackage(t *testing.T) {
	if cpgRunnerPackage != "com.lattice.security.Runner" {
		t.Fatalf("cpgRunnerPackage = %q, want com.lattice.security.Runner", cpgRunnerPackage)
	}
	// Ensure class path layout matches Java package.
	wantSuffix := filepath.Join("com", "lattice", "security", "Runner.class")
...
func TestBundledJavaHomeDir(t *testing.T) {
	...
	expected := filepath.Join(home, ".lattice", "java", "jdk-17")
...
func TestCpgRunnerSourcePath(t *testing.T) {
	...
	cpgDir := filepath.Join(tempDir, "cpg", "src", "main", "java", "com", "lattice", "security")
	...
	if err := os.WriteFile(runnerFile, []byte("package com.lattice.security;"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	expected := filepath.Join("cpg", "src", "main", "java", "com", "lattice", "security", "Runner.java")
