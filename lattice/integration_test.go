...
func repoRoot(t *testing.T) string {
	...
	// lattice/integration_test.go -> repo root
...
func TestIntegrationCPGRunnerPackageMatchesSource(t *testing.T) {
	...
	if err != nil {
		// Source only present when running from lattice working directory.
	...
	if !strings.Contains(string(data), "package com.lattice.security") {
...
func TestIntegrationOpenGrepScansVulnerablePHP(t *testing.T) {
	...
	// Local rules if present in lattice/rules; otherwise allow refresh.
	rulesDir := filepath.Join(root, "lattice", "rules")
	useLocalRules := false
