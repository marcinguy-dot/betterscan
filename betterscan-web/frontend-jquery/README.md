# BetterScan jQuery frontend (zero-build alternative)

A plain static frontend for BetterScan built with **jQuery + Bootstrap** and no
...
2. The returned JWT is stored in `localStorage` (`betterscan_token`).
...
```js
window.BETTERSCAN_API = "http://localhost:8080";
```
