...
├── worker/            # Redis consumer: clone (auth optional) + betterscan
...
- **Scan Execution**: Queue scans to Redis; worker clones (authenticated when needed) and runs betterscan
...
GitHub App env: `GITHUB_APP_ID`, `GITHUB_APP_SLUG`, `GITHUB_APP_PRIVATE_KEY` (or `_PATH`), callback `BETTERSCAN_PUBLIC_URL/api/v1/vcs/github/callback`. Local default: `GITHUB_APP_MOCK=1`.
...
### Worker
- **Go 1.21+**
- **betterscan** scanner integration
...
git clone https://github.com/betterscan-io/betterscan.git
cd betterscan-web
...
- `BETTERSCAN_PUBLIC_URL`, `BETTERSCAN_UI_URL` — callback and post-install redirects
...
### Worker
- `BETTERSCAN_PATH` - Path to betterscan binary
