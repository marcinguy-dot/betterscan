...
const (
	...
	cpgRunnerPackage = "com.betterscan.security.Runner"
)

var cpgRunnerSource = filepath.Join("cpg", "src", "main", "java", "com", "betterscan", "security", "Runner.java")
...
func cpgHomeDir() (string, error) {
	...
	return filepath.Join(home, ".betterscan", "cpg", cpgVersion), nil
}
...
func bundledJavaHomeDir() (string, error) {
	...
	return filepath.Join(home, ".betterscan", "java", fmt.Sprintf("jdk-%d", minJavaVersion)), nil
}
...
func downloadJavaJDK() (string, string, error) {
	...
	tempDir, err := os.MkdirTemp("", "betterscan-java-*")
	if err != nil {
		return "", "", err
	}
...
func hasSupportedCpgSources(codeDir string) (bool, error) {
	...
	err := filepath.WalkDir(codeDir, func(path string, d os.DirEntry, err error) error {
		...
		if d.IsDir() {
			name := d.Name()
			if name == ".git" || name == ".betterscan" || name == "node_modules" {
...
func httpGetWithRetry(url string) (*http.Response, error) {
	...
	for i := 0; i < 5; i++ {
		...
		req.Header.Set("User-Agent", "betterscan/1.0.0 (https://github.com/marcinguy/checkmate-go)")
