...
func TestCpgHomeDir(t *testing.T) {
	...
	expected := filepath.Join(home, ".betterscan", "cpg", cpgVersion)
	got, err := cpgHomeDir()
	if err != nil {
...
func TestCpgRunnerPackage(t *testing.T) {
	...
	wantSuffix := filepath.Join("com", "betterscan", "security", "Runner.class")
...
func TestBundledJavaHomeDir(t *testing.T) {
	...
	expected := filepath.Join(home, ".betterscan", "java", "jdk-17")
...
func TestCpgRunnerSourcePath(t *testing.T) {
	...
	cpgDir := filepath.Join(tempDir, "cpg", "src", "main", "java", "com", "betterscan", "security")
	...
	if err := os.WriteFile(runnerFile, []byte("package com.betterscan.security;"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	expected := filepath.Join("cpg", "src", "main", "java", "com", "betterscan", "security", "Runner.java")
