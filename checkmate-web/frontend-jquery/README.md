# Checkmate jQuery frontend (zero-build alternative)

A plain static frontend for Checkmate built with **jQuery + Bootstrap** and no
...
2. The returned JWT is stored in `localStorage` (`checkmate_token`).
...
```js
window.CHECKMATE_API = "http://localhost:8080";
```
