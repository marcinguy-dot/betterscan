...
func repoRoot(t *testing.T) string {
	...
	// go-checkmate/integration_test.go -> repo root
...
func TestIntegrationCPGRunnerPackageMatchesSource(t *testing.T) {
	...
	if err != nil {
		// Source only present when running from go-checkmate working directory.
	...
	if !strings.Contains(string(data), "package com.checkmate.security") {
...
func TestIntegrationOpenGrepScansVulnerablePHP(t *testing.T) {
	...
	// Local rules if present in go-checkmate/rules; otherwise allow refresh.
	rulesDir := filepath.Join(root, "go-checkmate", "rules")
	useLocalRules := false
