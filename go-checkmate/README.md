...
## Features
- Runs common Checkmate tools (OpenGrep, Trivy, Bandit, Brakeman, GoStaticcheck, Joern, Fraunhofer CPG)
- Two strategies:
...
Checkmate wires **two** code-property-graph style engines. They are complementary, not duplicates:

| Engine | Role in Checkmate | Strength |
|--------|-------------------|----------|
...
- Findings can still land on the same `file:line`; with `--dedupe-scope file-line`, Checkmate collapses cross-tool duplicates and keeps the longest message.

Fraunhofer positions the CPG as a language-independent analysis **platform** (query API, Neo4j export, library embed, compliance via Codyze)—highly extensible with custom language frontends, passes, and analyses—rather than a single fixed “scanner ruleset.” Checkmate’s `cpg` integration follows that model: a small built-in starter suite (e.g. NPE / OOB) plus room to grow **your** rules.
...
The `joern` tool runs [Joern](https://joern.io/) scan queries against your code directory (auto-install under `~/.checkmate/joern` when `--install-missing` is set). Prefer it for **excellent existing graph rules** and known vulnerability query packs.

```bash
./checkmate-go --tools joern --code-dir /path/to/project
```
